...
use analyze::{analyze_text, add_normalized_entropy, calculate_batch_extended_stats};
...
use correlation::{calculate_correlation_with_filter, plot_correlation_scatter_with_params, plot_dual_author_correlation_with_filter, detect_outliers_in_data, FilterParams, PlotParams};
use multi_poem::{analyze_multi_poem, analyze_two_authors, create_analysis_table, write_analysis_table_csv, print_analysis_table, plot_normal_distribution, TwoAuthorsData};
...
fn run_batch(args: BatchArgs) -> Result<()> {
    ...
    for entry in walker {
        ...
        let path = entry.path();
        if path.extension().and_then(|s| s.to_str()).unwrap_or("") != args.ext { continue; }
    ...
    if args.show_extended_stats {
        if let Some(ext_stats) = calculate_batch_extended_stats(&rows) {
            ...
            writeln!(stdout, "Median:      {:.6}", ext_stats.median.unwrap_or(0.0))?;
            ...
            writeln!(stdout, "Std Dev:     {:.6}", ext_stats.std_dev.unwrap_or(0.0))?;
            writeln!(stdout, "Variance:    {:.6}", ext_stats.variance.unwrap_or(0.0))?;
            writeln!(stdout, "CV (%):      {:.2}", ext_stats.coefficient_of_variation.unwrap_or(0.0))?;
            writeln!(stdout, "Skewness:    {:.6}", ext_stats.skewness.unwrap_or(0.0))?;
            writeln!(stdout, "Kurtosis:    {:.6}", ext_stats.kurtosis.unwrap_or(0.0))?;
            writeln!(stdout, "")?;
        }
    }
    ...
    for (file, s) in &rows {
        writeln!(stdout, "{},{},{},{},{}", file, s.n_words, fmt_opt(s.h_bits), fmt_opt(s.x_mean), fmt_opt(s.sigma))?;
    }
    ...
    if args.correlation || args.correlation_plot.is_some() {
        ...
        if let Some(corr_result) = calculate_correlation_with_filter(&rows, params.filter) {
            writeln!(stdout, "\nCorrelation Analysis:")?;
            if args.min_entropy.is_some() || args.max_entropy.is_some() || args.min_n_words.is_some() || args.max_n_words.is_some() {
            ...
            if args.label_top_entropy.is_some() {
                writeln!(stdout, "Labeling top {} texts by entropy", args.label_top_entropy.unwrap())?;
            }
            writeln!(stdout, "Samples: {}", corr_result.n_samples)?;
            writeln!(stdout, "Correlation coefficient (r): {:.6}", corr_result.correlation_coefficient)?;
            writeln!(stdout, "R-squared: {:.6}", corr_result.r_squared)?;
            writeln!(stdout, "Linear regression: H = {:.6} * N + {:.6}", corr_result.slope, corr_result.intercept)?;
    ...
    // file outputs
    if let Some(p) = args.summary_csv.as_deref() { write_summary_csv(p, &rows)?; }
    if let Some(p) = args.summary_json.as_deref() { write_summary_json(p, &rows)?; }
    Ok(())
}
...
fn run_multi_poem(args: MultiPoemArgs) -> Result<()> {
    ...
    if let Some(second_file) = &args.second_file {
        ...
        let two_authors = analyze_two_authors(&text, &text2, &norm, &args.delimiter, "Автор 1", "Автор 2")?;
        ...
        let mut stdout = io::stdout();
        writeln!(stdout, "\nАнализ {} (файл: {}):", two_authors.author1_name, args.input)?;
        print_analysis_table(&analysis_table1)?;
        
        writeln!(stdout, "\nАнализ {} (файл: {}):", two_authors.author2_name, second_file)?;
        ...
        let rows: Vec<(String, model::Summary)> = poems.iter()
...
fn run_single_author_analysis(
    ...
    writeln!(stdout, "file,N,H_bits,x_mean,sigma")?;
    for (title, s) in rows {
        writeln!(stdout, "{},{},{},{},{}", title, s.n_words, fmt_opt(s.h_bits), fmt_opt(s.x_mean), fmt_opt(s.sigma))?;
    }
    ...
    if let Some(normal_plot_path) = args.normal_dist_plot.as_deref() {
        ...
        writeln!(stdout, "Normal distribution plot saved to: {}", normal_plot_path.display())?;
    }
...
fn run_two_author_correlation(args: &MultiPoemArgs, two_authors: &TwoAuthorsData) -> Result<()> {
    ...
    let rows1: Vec<(String, model::Summary)> = two_authors.author1.iter()
        .map(|poem| (format!("{} - {}", two_authors.author1_name, poem.title), poem.summary.clone()))
        .collect();
        
    let rows2: Vec<(String, model::Summary)> = two_authors.author2.iter()
        .map(|poem| (format!("{} - {}", two_authors.author2_name, poem.title), poem.summary.clone()))
        .collect();
    ...
    if args.correlation {
        ...
        if let Some(corr_result1) = calculate_correlation_with_filter(&rows1, params.filter) {
            writeln!(stdout, "\nCorrelation Analysis for {}:", two_authors.author1_name)?;
            writeln!(stdout, "Samples: {}", corr_result1.n_samples)?;
            writeln!(stdout, "Correlation coefficient (r): {:.6}", corr_result1.correlation_coefficient)?;
            writeln!(stdout, "R-squared: {:.6}", corr_result1.r_squared)?;
            writeln!(stdout, "Linear regression: H = {:.6} * N + {:.6}", corr_result1.slope, corr_result1.intercept)?;
        }
        
        if let Some(corr_result2) = calculate_correlation_with_filter(&rows2, params.filter) {
            writeln!(stdout, "\nCorrelation Analysis for {}:", two_authors.author2_name)?;
            writeln!(stdout, "Samples: {}", corr_result2.n_samples)?;
            writeln!(stdout, "Correlation coefficient (r): {:.6}", corr_result2.correlation_coefficient)?;
            writeln!(stdout, "R-squared: {:.6}", corr_result2.r_squared)?;
            writeln!(stdout, "Linear regression: H = {:.6} * N + {:.6}", corr_result2.slope, corr_result2.intercept)?;
        }
    }
    
    // Combined dual-author correlation plot
    if let Some(plot_path) = args.correlation_plot.as_deref() {
        plot_dual_author_correlation_with_filter(plot_path, &rows1, &rows2, &two_authors.author1_name, &two_authors.author2_name, params.filter)?;
        writeln!(stdout, "Dual-author correlation plot saved to: {}", plot_path.display())?;
    }
    
    Ok(())
}

fn fmt_opt(v: Option<f64>) -> String { v.map(|x| format!("{:.6}", x)).unwrap_or_default() }
